...
- [ ] **P0-T1 — Init Astro** — `npm create astro@latest` (minimal template, TypeScript `strict`;
  Astro 6 requires **Node ≥ 22.12** — use a current Node LTS that satisfies it).
- [ ] **P0-T2 — Install all deps** — every package in the inventory above, `@latest`, in one pass.
- [ ] **P0-T3 — Configure `astro.config.mjs`** — add `vue()`, `sitemap()`, `site`, the
  `@tailwindcss/vite` plugin; `output: 'static'`. (Use `<ClientRouter />` in the layout, Phase 1.)
- [ ] **P0-T4 — Configure Tailwind v4** — `src/styles/global.css` with `@import "tailwindcss";`
  + a `@theme { … }` block scaffolding the design tokens (values filled in Phase 2).
- [ ] **P0-T5 — Configure shadcn-vue** — `npx shadcn-vue@latest init`; set `@/*` → `src/*` in
  `tsconfig.json`; create `src/lib/utils.ts` (`cn`); verify `components.json` aliases.
- [ ] **P0-T6 — Fonts & icons** — import `@fontsource-variable/outfit`; confirm `@lucide/vue` resolves.
- [ ] **P0-T7 — Pre-wire later-phase libs** — confirm `img-comparison-slider`, `glightbox`,
  `lite-youtube-embed`, `@calcom/embed-snippet` import without error (scratch file), then remove it.
- [ ] **P0-T8 — Dev tooling** — Prettier (+ astro & tailwind plugins; `.prettierignore` excludes the
...
- [ ] **P0-T9 — Cloudflare Pages config** — connect repo; build command `astro build`, output
...
- [ ] **P0-T10 — Verify & lock** — `npx npm-check-updates -u` (ncu = npm-check-updates; refresh)
