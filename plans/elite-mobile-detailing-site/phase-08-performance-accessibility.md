...
- [x] **P8-T1 — Hydration audit** — every island uses the lightest viable directive
...
- [x] **P8-T2 — Image strategy** — AVIF/WebP, dimensions set, lazy below fold. The hero is a single
...
- [x] **P8-T3 — Third-party lazy** — Cal.com + video facades load on demand; analytics deferred.
- [x] **P8-T4 — Fonts** — Outfit variable preloaded with `crossorigin` (mandatory on Safari even
  same-origin; preload URL must match the `@font-face src`; woff2-only), `swap`, latin subset; verify no CLS.
- [x] **P8-T5 — Lighthouse (mobile)** — run; fix to LCP < 2.5s, CLS < 0.1, low TBT; perf ≥ 90.
- [x] **P8-T6 — Accessibility (AA)**
...
- [x] **P8-T7 — Cross-browser / responsive** — Chrome/Safari/Firefox; mobile-first breakpoints.
