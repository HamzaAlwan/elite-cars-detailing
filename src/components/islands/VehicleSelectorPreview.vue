...
<template>
  <div class="space-y-14">
    ...
    <div id="car-variant-a">
      <p class="mb-1 text-xs font-semibold tracking-widest text-brand uppercase">Variant A</p>
      <h3 class="text-lg font-semibold text-text">Spring Bounce</h3>
      <p class="mt-1 mb-5 text-sm text-text-muted">
      ...
      >
        <ToggleGroupItem
          ...
          class="flex flex-1 cursor-pointer flex-col items-center gap-2 rounded-xl border border-border bg-bg-elevated px-2 py-3 text-text-muted transition-colors select-none hover:border-border-bright data-[state=on]:border-brand data-[state=on]:bg-brand-tint data-[state=on]:text-brand"
        >
          <div class="flex h-10 w-full items-end justify-center">
            <svg class="spring-car h-full w-auto" viewBox="0 0 120 60" aria-hidden="true">
              <g v-if="size.value === 'sedan'">
                <path
                  d="M 8 46 L 8 40 C 8 35 11 31 17 29 L 26 21 C 32 14 37 11 43 11
                         L 75 11 C 81 11 85 14 88 21 L 94 29 L 101 29
                         C 105 29 109 32 109 38 L 109 46
                         L 97 46 A 8 8 0 0 0 87 46
                         L 33 46 A 8 8 0 0 0 23 46 Z"
                  fill="currentColor"
                />
                <path
                  d="M 37 46 L 43 13 L 74 13 L 80 21 L 87 29 Z"
                  fill="currentColor"
                  class="car-window"
                />
              ...
              <g v-else-if="size.value === 'suv'">
                <path
                  d="M 6 48 L 6 41 C 6 36 9 31 15 29 L 22 21 C 26 16 30 12 36 11
                         L 78 11 C 84 11 88 14 91 19 L 96 29 L 105 29
                         C 108 29 112 33 112 40 L 112 48
                         L 99 48 A 9 9 0 0 0 85 48
                         L 33 48 A 9 9 0 0 0 19 48 Z"
                  fill="currentColor"
                />
                <path
                  d="M 30 48 L 36 13 L 77 13 L 83 21 L 90 29 Z"
                  fill="currentColor"
                  class="car-window"
                />
              ...
              <g v-else>
                <path
                  d="M 6 48 L 6 41 C 6 36 9 31 14 28 L 18 20 C 22 14 26 11 32 11
                         L 50 11 C 52 11 54 14 54 20 L 54 28 L 54 48
                         L 31 48 A 9 9 0 0 0 17 48 Z"
                  fill="currentColor"
                />
                <path
                  d="M 16 28 C 20 20 24 15 32 13 L 48 13 L 50 20 L 50 28 Z"
                  fill="currentColor"
                  class="car-window"
                />
                <path
                  d="M 58 30 L 114 30 C 116 30 117 32 117 36 L 117 48
                         L 97 48 A 9 9 0 0 0 83 48 L 58 48 Z"
                  fill="currentColor"
                />
    ...
    <div id="car-variant-b">
      <p class="mb-1 text-xs font-semibold tracking-widest text-brand uppercase">
        Variant B ✓ Chosen
      </p>
      <h3 class="text-lg font-semibold text-text">Drive In</h3>
      <p class="mt-1 mb-5 text-sm text-text-muted">
        The new car slides in from the appropriate side. Vue &lt;Transition&gt; + direction-aware
        CSS.
      </p>
      <div
        class="relative mb-4 h-28 overflow-hidden rounded-xl border border-brand/30 bg-bg-elevated"
      >
        <Transition :name="driveDirection === 'right' ? 'drive-right' : 'drive-left'" mode="out-in">
          <div
            :key="selectedB"
            class="absolute inset-0 flex items-end justify-center pb-2 text-brand"
          >
      ...
      >
        <ToggleGroupItem
          ...
          :value="size.value"
          :aria-label="size.label"
          class="flex-1 cursor-pointer rounded-xl border border-border bg-bg-elevated py-2.5 text-sm font-semibold text-text-muted transition-colors select-none data-[state=on]:border-brand data-[state=on]:bg-brand-tint data-[state=on]:text-brand"
        >
          {{ size.short }}
        </ToggleGroupItem>
      </ToggleGroupRoot>
    ...
    <div id="car-variant-c">
      <p class="mb-1 text-xs font-semibold tracking-widest text-brand uppercase">Variant C</p>
      ...
      >
        ...
        >
          <div class="flex h-12 w-full items-end justify-center overflow-visible">
            <svg
              class="glow-svg h-10 w-auto overflow-visible"
              viewBox="0 0 120 62"
              aria-hidden="true"
            >
              <g v-if="size.value === 'sedan'">
...
@keyframes car-spring {
  ...
  20% {
    transform: scale(1.07, 0.94) translateY(-5px);
  }
  50% {
...
.drive-right-leave-to {
  transform: translateX(-64px);
  opacity: 0;
}
.drive-right-enter-from {
  transform: translateX(64px);
  opacity: 0;
}
